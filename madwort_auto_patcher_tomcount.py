...
for	jacktrip_client in jacktrip_clients:
	...
	if len(mpg123_ports) == 1:
		p.connect_to_centre(jackClient, mpg123_ports[0], jacktrip_client)

	if len(mpg123_ports) >= 2:
		p.connect_to_left(jackClient, mpg123_ports[0], jacktrip_client)
		p.connect_to_right(jackClient, mpg123_ports[1], jacktrip_client)

	if len(mpg123_ports) == 3:
		p.connect_to_centre(jackClient, mpg123_ports[2], jacktrip_client)

	if len(mpg123_ports) >= 4:
		p.connect_to_soft_left(jackClient, mpg123_ports[2], jacktrip_client)
		p.connect_to_soft_right(jackClient, mpg123_ports[3], jacktrip_client)

	if len(mpg123_ports) == 5:
		p.connect_to_centre(jackClient, mpg123_ports[4], jacktrip_client)
